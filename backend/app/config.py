...
class Settings(BaseSettings):
    ...
    # 高德地图API配置 (自动加载 AMAP_MAPS_API_KEY 环境变量)
    ...
    # LLM配置
    llm_timeout: int = 120  # LLM请求超时时间(秒)
...
def validate_config():
    ...
    if not settings.amap_maps_api_key:
        errors.append("AMAP_MAPS_API_KEY未配置")
...
def print_config():
    ...
    print(f"高德地图API Key: {'已配置' if settings.amap_maps_api_key else '未配置'}")
